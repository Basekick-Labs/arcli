...
- **No state on disk besides the config file.** No history file, no cache. The one identity arcli carries is `installation_id` (random UUID minted by `config.Save()`, stored in the config file), sent as `Arcli-Installation-Id` so Arc's own opt-out telemetry can count CLI installations per instance (the same id goes to every server the user configures, and the README's Privacy section says so). Opt-outs: `send_installation_id = false`, `DO_NOT_TRACK=1`.
- **Every command talks only to the Arc endpoints the user configures, except `arcli sample`.** Since 26.09.4 the sample commands fetch datasets from `samples.basekick.net`, a Basekick-controlled host, and carry the same User-Agent and installation id as any other request (the opt-outs above apply), so a download can be correlated with the Arc instance it was loaded into. This was the user's explicit decision; do not "fix" it back into a separate anonymous client. The sample requests never carry the token, the Arc endpoint, or anything about the user's data.
- **Do not write "arcli never phones home" or "only ever talks to the Arc endpoints you configure" as an unqualified claim** in the README, the docs site, or command help. It stopped being true in 26.09.4. State the `arcli sample` exception, and say that Cloudflare's platform logging records IP/path/UA/timestamp for those downloads rather than claiming nothing is recorded. Anything *beyond* the sample host still needs an explicit decision, an opt-in, and a doc rewrite.
